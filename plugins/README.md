...
本目录包含小说书库的三套 IDE 阅读插件。它们共享同一套本机协议和阅读语义，但分别使用各 IDE 的原生扩展机制实现界面与编辑器集成。插件可以连接桌面端 Bridge，也可以按需启动共享本地 Runtime 独立维护书库。
...
本地模式下三端还统一提供 TXT/EPUB 后台导入、可选受管源文件、删除、重新解析、备份恢复、桌面/本地显式迁移和 Runtime 诊断。用户可以自定义本地数据目录；安全复制后书库的持久 `storageId` 保持不变。Runtime 安装到用户级共享版本目录，三端通过安装锁、活动版本和防降级规则共同维护。
