  ...
  <section class="workspace-view book-detail-view">
    ...
    <template v-else-if="book">
      <header class="book-detail-header" :style="{ '--book-accent': book.theme.accent, '--book-bg': book.theme.background, '--book-text': book.theme.text }">
        <div class="book-detail-seal">{{ book.title.slice(0, 1) }}</div>
        <div><p>{{ book.chapterCount }} CHAPTERS · {{ book.totalWords.toLocaleString() }} WORDS</p><h1>{{ book.title }}</h1><span>{{ book.author || '佚名' }}</span><blockquote>{{ book.description || '暂无简介' }}</blockquote><div class="header-actions"><button type="button" class="primary-command" @click="router.push(`/read/${book.id}/${book.currentChapter}`)"><BookOpen :size="17" />{{ book.progress ? '继续阅读' : '开始阅读' }}</button><button type="button" class="secondary-command danger-command" @click="removeBook"><Trash2 :size="16" />删除</button></div></div>
      </header>

      <section class="chapter-catalogue">
        <header><p>TABLE OF CONTENTS</p><h2>章节目录</h2></header>
        <section v-for="group in chapterGroups" :key="group.name" class="chapter-group"><h3>{{ group.name }} <span>{{ group.items.length }} 章</span></h3><div><button v-for="chapter in group.items" :key="chapter.id" type="button" @click="router.push(`/read/${book.id}/${chapter.number}`)"><small>{{ formatChapterLabel(chapter) }}</small><span>{{ chapter.title }}</span><ChevronRight :size="15" /></button></div></section>
      </section>
    </template>
  </section>
</template>
