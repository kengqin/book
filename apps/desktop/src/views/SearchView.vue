...
<template>
  <section class="workspace-view">
    ...
    <div v-else class="search-results"><button v-for="result in results" :key="`${result.bookId}:${result.chapterNumber}`" type="button" @click="router.push(`/read/${result.bookId}/${result.chapterNumber}`)"><div><small>{{ result.bookTitle }} · 第 {{ result.chapterNumber }} 章</small><strong>{{ result.chapterTitle }}</strong><p>{{ result.snippet }}</p></div><ArrowRight :size="17" /></button></div>
  </section>
</template>
