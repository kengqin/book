...
const plugin = { version: '0.4.6' } as BundledIdePlugin
...
describe('IDE plugin updates', () => {
  it('orders numeric version segments', () => {
    expect(comparePluginVersions('0.4.5', '0.4.6')).toBe(-1)
    expect(comparePluginVersions('0.4.10', '0.4.6')).toBe(1)
    expect(comparePluginVersions('v0.4.6', '0.4.6')).toBe(0)
  })

  it('offers an update only for an installed older version', () => {
    expect(idePluginUpdateAvailable(target('0.4.5'), plugin)).toBe(true)
    expect(idePluginUpdateAvailable(target('0.4.6'), plugin)).toBe(false)
    expect(idePluginUpdateAvailable(target('0.4.7'), plugin)).toBe(false)
    expect(idePluginUpdateAvailable(target(), plugin)).toBe(false)
    expect(idePluginUpdateAvailable(target('0.4.5', false), plugin)).toBe(false)
  })
})
