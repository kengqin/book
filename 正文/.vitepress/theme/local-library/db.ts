...
import type { LocalAsset, LocalBook, LocalChapter, ParseOptions, ParseResult, ThemeSettings } from './types'
...
export async function updateBookProgress(bookId: string, chapter: number, progress: number) {
  ...
  if (!book) return
  const overallProgress = Math.min(100, ((chapter - 1) + progress / 100) / Math.max(1, book.chapterCount) * 100)
...
export async function importLibraryBackup(payload: any) {
  if (payload?.version !== 1 || !Array.isArray(payload.books) || !Array.isArray(payload.chapters)) throw new Error('不是有效的本地书架备份')
  const db = await getDB()
  const tx = db.transaction(['books', 'chapters', 'assets'], 'readwrite')
  for (const book of payload.books) tx.objectStore('books').put(book)
  for (const chapter of payload.chapters) tx.objectStore('chapters').put(chapter)
  for (const asset of payload.assets ?? []) tx.objectStore('assets').put({ ...asset, blob: dataURLToBlob(asset.blob) })
  await tx.done
}
