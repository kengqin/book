...
import { defaultParseOptions, defaultTheme, formatChapterLabel, type LocalBook, type ParseOptions, type ParseResult, type ParserResponse, type ThemeSettings } from './types'
      ...
      <section class="local-modal import-modal" role="dialog" aria-modal="true" aria-label="导入 TXT 小说">
        ...
        <div v-else-if="step === 2" class="import-step-body">
          ...
          <template v-else>
            ...
            <div class="chapter-preview"><aside><button v-for="(chapter, index) in result.chapters.slice(0, 80)" :key="chapter.number" type="button" :class="{ active: previewIndex === index }" @click="previewIndex = index"><small>{{ chapter.volume || '正文' }}</small><span>{{ formatChapterLabel(chapter) }} {{ chapter.title }}</span></button><p v-if="result.chapters.length > 80">其余 {{ result.chapters.length - 80 }} 章将在导入后显示</p></aside><article v-if="selectedChapter"><small>格式预览 · {{ selectedChapter.wordCount }} 字</small><h3>{{ formatChapterLabel(selectedChapter) }} {{ selectedChapter.title }}</h3><p v-for="paragraph in selectedChapter.content.split(/\n{2,}/).slice(0, 12)" :key="paragraph">{{ paragraph }}</p></article></div>
