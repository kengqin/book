...
import type { ParseOptions, ParsedChapter, ParserRequest, ParserResponse, TextEncoding } from './types'
...
function detectEncoding(buffer: ArrayBuffer, requested: TextEncoding): Exclude<TextEncoding, 'auto'> {
...
function parse(request: ParserRequest) {
  send({ type: 'progress', progress: 8, message: '正在识别文本编码' })
  ...
  send({ type: 'progress', progress: 28, message: '正在识别卷与章节' })
  ...
  send({ type: 'progress', progress: 52, message: `正在整理 ${headers.length} 个章节` })
  ...
  send({ type: 'progress', progress: 92, message: '正在生成预览' })
