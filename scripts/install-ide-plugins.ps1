...
function Package-VSCode {
  ...
  $output = Join-Path $artifactRoot 'novel-library-reader-0.4.1.vsix'
...
if ($Only -in @('All', 'VSCode')) {
  try {
    $plugin = if ($SkipBuild) { Join-Path $artifactRoot 'novel-library-reader-0.4.1.vsix' } else { Package-VSCode }
