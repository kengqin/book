...
- `书库/` 保存平台发布的书籍、章节、元数据和专题资源。
- `scripts/generate-library.mjs` 在构建前扫描书库并生成发布清单。
- `正文/` 使用 VitePress 构建在线阅读站点。
- `正文/.vitepress/theme/local-library/` 提供浏览器本地 TXT 导入、章节解析、书架、阅读进度、主题和备份功能。
...
正文/                        # 现有 VitePress 在线端，逐步改用 packages
书库/                        # 现有发布内容，位置暂不调整
scripts/                     # 现有构建脚本
docs/                        # 架构、数据和发布文档
```

第一阶段不强制将 `正文/` 移入 `apps/web/`，避免同时改变 VitePress 的 `srcDir`、资源路径和 GitHub Pages 发布路径。等共享包稳定后，再单独评估目录迁移。
...
- SQLite schema v2 与自动迁移已启用。
- 数据目录固定为 `%APPDATA%\NovelLibrary`。
- TXT 导入、书架、目录、阅读、章内进度、搜索和删除链路已接通。
- JSON 备份恢复已接通。
- NSIS 安装包已生成。
