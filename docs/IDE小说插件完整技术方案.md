...
> 后续“插件脱离桌面端独立安装和运行”的双模式架构见 [`IDE插件独立运行与桌面端双模式完整方案.md`](./IDE插件独立运行与桌面端双模式完整方案.md)。涉及共享本地 Runtime、数据源切换、独立存储和插件市场发布的内容，以该独立化方案为准。
...
- 桌面端与三个 IDE 插件独立维护版本；插件只有自身代码或协议发生兼容性变更时才升级。当前桌面端为 `0.5.0`，三个插件制品为 `0.5.2`，两者不是发布约束。
