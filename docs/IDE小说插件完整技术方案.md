...
使用 CommonJS 扩展。`0.4.2` 提供活动栏与侧边栏，侧边栏展示当前书籍、章节和 5 行正文，并提供书籍/章节选择、逐行与切章命令。编辑器通过行尾 Decoration 自动混入 5 行小说，状态栏同步显示阅读位置。Cursor 复用 VS Code 扩展协议。
...
```text
novel-library-reader-0.4.2.vsix
...
- Bridge 请求最长 5 秒，VS Code/Cursor 请求使用 `Connection: close`，避免桌面端重启时复用失效连接。
- 重复导入和 existingId 更新。
- 书签、笔记和进度同步。
...
- IntelliJ `0.4.1`、VS Code/Cursor `0.4.2`、Visual Studio `0.4.0` 完整适配器。
