...
使用 Kotlin + IntelliJ Platform Gradle Plugin。`0.4.1` 使用 Java 17 字节码、`since-build=241` 且不设置上限，一个插件覆盖 IDEA、PyCharm、WebStorm、Android Studio、Rider、CLion、GoLand 和 RubyMine。插件提供 Tool Window、Action、当前文件导入、5 行代码行尾 Inlay、启动自动加载和 Bridge 客户端。

### 11.2 VS Code / Cursor

使用 CommonJS 扩展。`0.4.3` 提供活动栏与分层侧边栏：书架默认展开，章节目录可折叠，正文默认展开并固定展示当前 5 行；当前书籍和章节有明确标记，点击条目即可切换。显示隐藏、逐行、切章和刷新使用视图标题栏 Codicon，命令与快捷键继续保留但不再占用阅读内容区域。编辑器通过行尾 Decoration 自动混入 5 行小说，状态栏同步显示阅读位置。Cursor 复用 VS Code 扩展协议。

### 11.3 Visual Studio

使用 Visual Studio 2022 VSIX 工程和 C# Bridge 客户端。`0.4.0` 提供 WPF 工具窗口、书籍/章节选择、5 行阅读、快捷键和编辑器 `IAdornmentLayer` 行尾显示。VSSDK 生成正式 `.pkgdef` 和命令表，`scripts/package-visual-studio-plugin.ps1` 只验证官方 VSIX，不再自行压缩伪造扩展包。
...
```text
novel-library-reader-0.4.3.vsix
novel-library-intellij-0.4.1.zip
novel-library-visual-studio-0.4.0.vsix
manifest.json
```
...
- IntelliJ `0.4.1`、VS Code/Cursor `0.4.3`、Visual Studio `0.4.0` 完整适配器。
