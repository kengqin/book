...
3. 发布前只要求待发布提交已经进入 `main`，再创建对应 Tag；不强制要求提交必须来自 `develop` 或使用特定合并父提交结构。
4. Tag 格式为 `vX.Y.Z`，且必须与所有版本文件一致。
5. 同一个版本不得重复创建不同内容的正式 Release。

`main` 是唯一发布授权分支。发布签名私钥和密码保存在 GitHub Actions Secrets，由 Tag 触发的发布工作流读取；功能分支和本地环境不需要 GitHub Release 密钥，本地 `gh` 登录也不是发布前置条件。正确流程是“工作分支推送 → 审核合并 `main` → 推送 `main` → 从 `main` 创建并推送 Tag → Actions 使用仓库 Secrets 构建、签名和发布”，禁止从工作分支直接打正式 Release Tag。
